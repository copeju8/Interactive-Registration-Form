...
/*Payment Section
Goal: Display payment sections based on the payment option chosen in the select menu

Classes were added to identify the correct siblings or element in html.
The event handler was create to listen for each selection or event in the "Payment Information" 
drop down menu.
*/
...
Goal: To account for user errors and permit the submission of the form with errors.

Classes and Ids were create for each type of error on the form.
Functions using jQuery tested each valid and non-valid condition (input fields or selection menus) either showing or hiding the messages, 
and called in the Form Validation function below. Regular expressions tested the validity of the passed in value.
*/
...
/*Form Validation
Goal: Check the validity of the user input in each required field on the form.
If-else statements checked the valid by calling the functions for each user field. 
Red color fonts were use to bring attention to the errors.
*/
const formValidation = () => {

    if ($("#payment").val() === "credit card") {
        if (isValidUserName($('#name').val()) && isValidEmail($('#mail').val()) &&
            isValidActivity() && isValidCreditCard($("#cc-num").val()) && isValidZipCode($("#zip").val()) && isValidCVV($("#cvv").val())) {
            return true;
        } else {
